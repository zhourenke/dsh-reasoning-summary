...
|---|---|---|---|
| `PROMPT` | `src/index.ts` | 无（README 只描述协议） | 每个 Session 首个 enabled、非 warmup 步骤的普通上下文消息；宿主随后将其写入正常 `user/message` 历史，后续步骤和 turn 不重复注入；插件重新激活时从 durable history 识别已有协议上下文 |
