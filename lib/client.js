...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        function SummaryCard(props) {
            ...
            const stale = catalogKeys === null ? draftModels : draftModels.filter((item) => !catalogKeys.has(keyOf(item)));
            ...
            }, e('span', { className: 'rs-heading' }, e('strong', null, t('title')), e('span', null, t('description'))), dirty ? e('span', { className: 'rs-pending' }, t('unsaved')) : null, e(IconChevronDownOutline14, { className: `rs-chevron ${open ? 'rs-chevron-open' : ''}` })), open ? e('div', { className: 'rs-body' }, !snapshot.writable ? e('p', { className: 'rs-readonly', role: 'status' }, t('readOnly')) : null, e('div', { className: 'rs-field rs-model-field' }, e('div', { className: 'rs-model-title' }, e('span', { className: 'rs-model-label' }, t('models')), e('span', { className: 'rs-count' }, t('selected', draftModels.length)), e('button', { type: 'button', className: 'rs-refresh', onClick: () => { void loadCatalog(); } }, t('refresh'))), e('p', { className: 'rs-hint' }, t('modelsHint')), catalogError !== null ? e('p', { className: 'rs-error' }, `${t('catalogFailed')} (${catalogError})`) : null, catalog === null && catalogError === null ? e('p', { className: 'rs-loading' }, t('loading')) : null, stale.map((item) => e('div', { className: 'rs-row rs-stale', key: `stale:${keyOf(item)}` }, e('label', { className: 'rs-row-main' }, e('input', { type: 'checkbox', checked: true, disabled: !snapshot.writable || saving, onChange: () => toggle(item) }), e('span', null, e('strong', null, `${item.provider} / ${item.model}`), e('small', null, t('unavailable')))), e('button', { type: 'button', className: 'rs-remove', disabled: !snapshot.writable || saving, onClick: () => cleanStale(item) }, t('cleanup')))), catalog?.map((group) => e('div', { className: 'rs-group', key: group.id }, e('div', { className: 'rs-provider' }, group.name ?? group.id), (group.models ?? []).map((model) => {
                const item = { provider: group.id, model: model.id };
                const checked = selected.has(keyOf(item));
                return e('label', { className: 'rs-row', key: keyOf(item) }, e('input', { type: 'checkbox', checked, disabled: !snapshot.writable || saving, onChange: () => toggle(item) }), e('span', { className: 'rs-row-main' }, e('strong', null, model.name && model.name !== model.id ? model.name : model.id), e('small', null, `${group.id} / ${model.id}`)));
            }))), catalog && catalog.length === 0 ? e('p', { className: 'rs-loading' }, t('noModels')) : null), e('div', { className: 'rs-footer' }, failed ? e('p', { className: 'rs-error' }, t('saveFailed')) : null, e('button', { type: 'button', className: 'rs-discard', disabled: !dirty || saving, onClick: discard }, t('discard')), e('button', { type: 'button', className: 'rs-save', disabled: !dirty || saving || !snapshot.writable, onClick: () => { void save(); } }, saving ? t('saving') : t('save')))) : null);
        }
        const css = `
