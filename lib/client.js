...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        function translator(props) {
            ...
            return (key, ...args) => {
                const value = language[key] ?? zh[key] ?? key;
        ...
        function SummaryCard(props) {
            ...
            const removeStale = (item) => {
                const key = keyOf(item);
                setDraftModels((current) => current.filter((entry) => keyOf(entry) !== key));
                setDirty(true);
            };
            const save = async () => {
            ...
            for (const item of draftModels) {
                if (catalogKeys !== null && catalogKeys.has(keyOf(item)))
                    continue;
                const host = (catalog ?? []).find((group) => group.id === item.provider);
                if (host) {
            ...
            const renderStaleRow = (item) => e('div', { className: 'rs-model', key: `stale:${keyOf(item)}` }, e('input', { type: 'checkbox', checked: true, disabled: !snapshot.writable || saving, onChange: () => toggle(item) }), e('span', null, e('span', { className: 'rs-model-name' }, `${item.provider} / ${item.model}`), e('span', { className: 'rs-route' }, t('unavailable'))), e('button', {
            ...
            }, e('span', { className: 'rs-heading' }, e('span', { className: 'rs-name' }, t('title')), e('span', { className: 'rs-description' }, t('description'))), dirty ? e(Tag, { tone: 'neutral', className: 'rs-pending' }, t('unsaved')) : null, e(IconChevronDownOutline14, { className: `rs-chevron ${open ? 'rs-chevron-open' : ''}` })), open ? e('div', { className: 'rs-body' }, !snapshot.writable ? e('p', { className: 'rs-readonly', role: 'status' }, t('readOnly')) : null, e('div', { className: 'rs-field' }, e('div', { className: 'rs-model-title' }, e('span', { className: 'rs-model-label' }, t('models')), e('span', { className: 'rs-count' }, t('selected', draftModels.length))), e('p', { className: 'rs-hint' }, t('modelsHint')), catalogError !== null ? e('div', { className: 'rs-catalog-error', role: 'alert' }, e('span', null, `${t('catalogFailed')} (${catalogError})`), e('button', { type: 'button', disabled: saving, onClick: () => { void loadCatalog(); } }, t('retry'))) : null, catalog === null && catalogError === null ? e('p', { className: 'rs-notice', role: 'status' }, t('loading')) : null, (catalog && catalog.length > 0) || draftModels.length > 0 ? e('fieldset', { className: 'rs-models' }, e('legend', null, t('models')), catalog?.map((group) => e('div', { className: 'rs-model-group', key: group.id }, e('div', { className: 'rs-provider' }, group.name ?? group.id), (group.models ?? []).map((model) => {
                const item = { provider: group.id, model: model.id };
                const checked = selected.has(keyOf(item));
                return e('div', { className: 'rs-model', key: keyOf(item) }, e('input', { type: 'checkbox', checked, disabled: !snapshot.writable || saving, onChange: () => toggle(item) }), e('span', null, e('span', { className: 'rs-model-name' }, model.name && model.name !== model.id ? model.name : model.id), e('span', { className: 'rs-route' }, `${group.id} / ${model.id}`)));
            }), (staleByProvider.get(group.id) ?? []).map(renderStaleRow))), orphanStale.length > 0 ? e('div', { className: 'rs-model-group' }, e('div', { className: 'rs-provider' }, t('unavailableGroup')), orphanStale.map(renderStaleRow)) : null) : null, catalog && catalog.length === 0 && draftModels.length === 0 ? e('p', { className: 'rs-notice' }, t('noModels')) : null), e('div', { className: 'rs-footer' }, failed ? e('p', { className: 'rs-failed', role: 'status' }, t('saveFailed')) : null, e('button', { type: 'button', className: 'rs-discard', disabled: !dirty || saving, onClick: discard }, t('discard')), e('button', { type: 'button', className: 'rs-save', disabled: !dirty || saving || !snapshot.writable, onClick: () => { void save(); } }, saving ? t('saving') : t('save')))) : null);
        }
        const css = `
