...
const MISSING_TEXT = 'Summary unavailable: the model did not provide a complete summary.';
...
function summaryTags(text) {
    ...
    for (const match of text.matchAll(SUMMARY_OPEN)) {
        if (match.index !== undefined)
            tags.push({ kind: 'open', index: match.index, end: match.index + match[0].length });
    }
    for (const match of text.matchAll(SUMMARY_CLOSE)) {
        if (match.index !== undefined)
            tags.push({ kind: 'close', index: match.index, end: match.index + match[0].length });
    }
 ...
 */
function normalizeTextBlocks(texts, required, _turn, _step, forcedStatus) {
...
function replaceTextChunks(chunks, normalized) {
    const output = [];
    ...
    for (const chunk of chunks) {
        ...
        if (chunk.type === 'block-end' && chunk.block.type === 'text') {
            ...
            if (text === undefined)
                ...
                omitted.add(chunk.index);
            else if (!omitted.has(chunk.index))
                output.push({ ...chunk, block: { type: 'text', text } });
            continue;
        }
        output.push(chunk);
    }
...
function hideToolStepText(chunks) {
    ...
    for (const index of textBlockIndexes(chunks))
        normalized.set(index, '');
...
function mergeSummaryDetails(summary, details) {
    const primary = summaryBaseContent(summary);
    if (!primary)
        return details;
    if (!details || primary === details)
        return primary;
    if (details.includes(primary))
        return details;
    if (primary.includes(details))
        return primary;
    return `${primary}\n${details}`;
}
...
function injectNextStep(state, message, allowClosedStep = false) {
    if (!state.active || state.relayScheduled || state.relayQueued || (!allowClosedStep && !state.stepOpen) || state.signal?.aborted)
        return false;
...
function appendFinalRelay(state, allowClosedStep = false) {
    ...
    if (!state.active || !message || state.relayScheduled || state.relayQueued || (!allowClosedStep && !state.stepOpen) || state.signal?.aborted || state.relayAttempts >= 2)
        return;
    state.relayQueued = true;
    state.relayAttempts++;
    try {
...
function queueFinalRelay(state) {
    if (!state.finalRelay || state.relayScheduled || state.relayQueued)
        return;
...
function queueToolRelay(state) {
    if (!state.active || !state.finalRelay || state.relayScheduled || state.relayQueued || !allToolResultsSettled(state))
        return;
    state.relayQueued = true;
...
function finishState(state, partial, terminal) {
    ...
    const normalized = normalizeTextBlocks(texts, true, state.turn, state.step, partial ? 'partial' : undefined);
...
const PROMPT = `Tool-step communication protocol\n\nWhen you will call one or more tools, emit exactly one literal XML-style summary tag immediately before the first tool call, then emit the tool call(s):\n<summary>target, concrete evidence or current state, and the immediate operation or decision</summary>\n\nThe summary is the execution record carried into the next step. Make it specific and actionable: name the relevant user request, artifact, observation, change, or decision. Include concrete file, function, command, result, or constraint details when they are needed to perform the next action. Do not replace facts with vague status language such as “continue analysis”, “make progress”, or “check the implementation”.\n\nIn a tool-calling step, emit no ordinary assistant prose outside that tag. Do not narrate internal planning, code paths, module identities, test mechanics, or implementation details as a standalone user-visible progress message. Put execution detail in the summary instead.\n\nFor a final answer with no tool call, emit no summary tag: provide one complete user-facing answer with the necessary context and conclusions. If you have neither a tool call nor a complete answer, do not emit a partial progress message; continue reasoning until you can do one of those. Never stop after reasoning alone.`;
...
export function apply(ctx) {
    ...
    on('system-prompt/assemble', async (assembly, assemblyContext, next) => {
