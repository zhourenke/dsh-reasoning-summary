...
插件会在响应结束前缓存被选中路由的输出，以便解析摘要、隐藏工具步骤普通文本，并保证 reasoning、usage、block 和工具帧按 Provider 原始接收顺序交给下游流处理插件（例如 reasoning-merge）。因此该路由本来就是伪非流式；这是为了保证输出协议和下游 block 顺序稳定。
